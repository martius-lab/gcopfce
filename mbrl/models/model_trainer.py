...
class ModelTrainer:
    ...
    Args:
        ...
        additional_modules (dict): additional modules to be trained, used
    ...
    def __init__(
        ...
        additional_modules: Optional[dict] = {},
    ):
        self.model = model
        ...
        self.logger = logger
        if self.logger:
            [MODEL_LOG_FORMAT.append(
                (f"{k}_loss", f"{k[:3].upper()}LOSS", "float")
            ) for k in additional_modules.keys()]
    ...
    def train(
        ...
        Args:
            ...
            train_model (bool): whether the dynamics model should be trained.

            additional_modules (dict): dict containing additional modules to train; might
                exclude those that are not to be trained in this phase (offline/online).

            additional_datasets (dict): dict containing datasets for training each of the
                additional modules.
        ...
        for epoch in epoch_iter:
            if not train_model and not len(additional_datasets):
                break
            if batch_callback:
            ...
            [m.on_epoch(epoch) for m in additional_modules.values()]
            module_loss = {
                k: np.mean(
                    [v.update(batch) for batch in tqdm.tqdm(additional_datasets[k], disable=disable_tqdm)]
                ).item() for k, v in additional_modules.items()
            }
            ...
            if self.logger and not silent:
                ...
                log_info.update({f"{k}_loss": v for k, v in module_loss.items()})
    ...
    variables_to_save = ['_train_iteration']

    def get_state(self):
        return {**{k: getattr(self, k) for k in self.variables_to_save}, 'optimizer_state': self.optimizer.state_dict()}
