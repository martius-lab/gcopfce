...
class GaussianMLP(Ensemble):
    ...
    def _forward_ensemble(
        ...
        if self.propagation_method == "random_model":
            ...
            model_indices = torch.randperm(x.shape[1], device=self.device, generator=rng)
    ...
    def sample_propagation_indices(
        ...
        return torch.randperm(batch_size, device=self.device, generator=_rng)
    ...
    def save(self, save_dir: Union[str, pathlib.Path]):
        ...
        torch.save(self.get_state(), pathlib.Path(save_dir) / self._MODEL_FNAME)

    def load(self, load_dir: Union[str, pathlib.Path]):
        """Loads the model from the given path."""
        path = pathlib.Path(load_dir) / self._MODEL_FNAME
        if path.exists():
            self.set_state(torch.load(pathlib.Path(load_dir) / self._MODEL_FNAME, map_location=self.device))

    def get_state(self):
        return {"state_dict": self.state_dict(), "elite_models": self.elite_models}

    def set_state(self, state):
        self.load_state_dict(state["state_dict"])
        self.elite_models = state["elite_models"]
