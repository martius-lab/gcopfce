...
class OneDTransitionRewardModel(Model):
    ...
    def __init__(
        ...
        self.output_normalizer: Optional[mbrl.util.math.Normalizer] = None
        if normalize:
    ...
    def _get_model_input(
        ...
        if with_items:
            return model_in, model_in[..., :obs.shape[-1]], model_in[..., obs.shape[-1]:]
        return model_in
    ...
    def _get_model_output(
        ...
        return self.output_normalizer.unnormalize(obs) if self.output_normalizer else obs

    def _process_batch(
        self, batch: mbrl.types.TransitionBatch, _as_float: bool = False
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        obs, action, next_obs, reward, _, _ = batch.astuple()
        model_in, obs, _ = self._get_model_input(obs, action, with_items=True)
        if self.output_normalizer:
            next_obs = self.output_normalizer.normalize(next_obs).float().to(self.device)        
    ...
    def update_normalizer(self, batch: mbrl.types.TransitionBatch):
        ...
        if self.output_normalizer:
            ...
            if obs.ndim == 1:
                obs = obs[None, :]
            self.output_normalizer.update_stats(obs)
    ...
    def sample(
        ...
        Args:
            ...
            return_distributions (bool, optional): whether to return means and stds.
        ...
        model_in, obs, _ = self._get_model_input(model_state["obs"], act, with_items=True)
        if not hasattr(self.model, "sample_1d"):
            raise RuntimeError(
                "OneDTransitionRewardModel requires wrapped model to define method sample_1d"
            )
        
        result = self.model.sample_1d(
            model_in, model_state, rng=rng, deterministic=deterministic, return_distributions=return_distributions,
        )
        if return_distributions:
            preds, next_model_state, mus, stds = result
            mus = mus[:, :-1] if self.learned_rewards else mus
            stds = stds[:, :-1] if self.learned_rewards else stds
        else:
            preds, next_model_state = result
        ...
        if self.target_is_delta:
            ...
            next_observs = tmp_
        next_observs = self._get_model_output(next_observs)
        rewards = preds[:, -1:] if self.learned_rewards else None
        next_model_state["obs"] = next_observs

        if return_distributions:
            return next_observs, rewards, None, next_model_state, mus, stds
    ...
    def save(self, save_dir: Union[str, pathlib.Path]):
        ...
        if self.input_normalizer:
            state['input_normalizer'] = self.input_normalizer.get_state()
        if self.output_normalizer:
            state['output_normalizer'] = self.output_normalizer.get_state()
        torch.save(state, pathlib.Path(save_dir) / self.model._MODEL_FNAME)

    def load(self, load_dir: Union[str, pathlib.Path]):
        if load_dir and (pathlib.Path(load_dir) / self._MODEL_FNAME).exists():
            state = torch.load(pathlib.Path(load_dir) / self._MODEL_FNAME, map_location=self.device)
            self.model.set_state(state['model'])
            if self.input_normalizer:
                self.input_normalizer.set_state(state['input_normalizer'])
            if self.output_normalizer:
                self.output_normalizer.set_state(state['output_normalizer'])
