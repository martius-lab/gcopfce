...
def complete_agent_cfg(
    ...
    if hasattr(env, 'goal_space'):
        _check_and_replace("num_inputs", obs_shape[0] + env.goal_space.shape[0], agent_cfg)
    else:
    ...
    if "action_space" in agent_cfg.keys() and isinstance(
        ...
        # in MBPO, the action space expects low and high as np.arrays
        agent_cfg.action_space.low = {'_target_': 'numpy.array', 'object': agent_cfg.action_space.low}
        agent_cfg.action_space.high = {'_target_': 'numpy.array', 'object': agent_cfg.action_space.high}
