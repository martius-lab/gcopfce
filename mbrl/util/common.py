...
def create_replay_buffer(
    ...
    Args:
        ...
        goal_shape (Sequence of ints): the shape of goal arrays.
        ...
        goal_type (type): the goal type of the rewards (defaults to np.float32).
        ...
        goal_proj_fn (Callable, optiona): a function projecting observations to goals.
...
def get_basic_buffer_iterators(
...
) -> Tuple[TransitionIterator, Optional[TransitionIterator], dict]:
    ...
    Args:
        ...
        relabeling_fn (Callable, optional): goal relabeling function.
        ...
        additional_modules (dict): dictionary containing each of the additional
            modules for computing costs, which are trained online or offline.
    Returns:
    ...
    additional_iters = {k: v.get_iter()(
            transitions=replay_buffer.get_all(shuffle=False),
            trajectory_indices=replay_buffer.get_trajectory_indices(),
            batch_size=batch_size,
            relabeling_fn=relabeling_fn,
            goal_proj_fn=replay_buffer.goal_proj_fn,
            rng=replay_buffer.rng,
        ) for k, v in additional_modules.items()}
    ...
    return train_iter, val_iter, additional_iters
...
def train_model_and_save_model_and_data(
    ...
    train_model: Optional[bool] = True,
    additional_modules: dict = {},
    relabeling_fn: Optional[Callable] = None
):
    ...
    Args:
        ...
        train_model (bool, optional): whether the dynamics model should be trained.
        additional_modules (dict): dictionary containing additional modules to train.
        relabeling_fn (callable, optional): reward computation function for goal relabeling.
    """
    dataset_train, dataset_val, additional_datasets = mbrl.util.common.get_basic_buffer_iterators(
        replay_buffer,
    ...
    if hasattr(model, "update_normalizer") and train_model:
        model.update_normalizer(replay_buffer.get_all())
    [m.update_normalizer(d[:d.num_stored]) for m, d in zip(additional_modules.values(), additional_datasets.values())]
    model_trainer.train(
        dataset_train,
    ...
    if work_dir is not None:
        ...
        [module.save(str(work_dir)) for k, module in model_trainer.additional_modules.items()]
...
def rollout_agent_trajectories(
    ...
    Args:
        ...
        grow_buffer (bool): whether new trajectories should be added to the buffer.
    ...
    if steps_or_trials_to_collect == 0:
        return 0.
    step = 0
    trial = 0
    ...
    while True:
        ...
        agent.reset()
        if reset_callback:
            kwargs_from_module = reset_callback(obs, replay_buffer)
        done = False
        total_reward = 0.0
        while not done:
            if replay_buffer is not None:
                next_obs, reward, done, info, kwargs_from_module = \
                ...
                action = agent.act(obs, **agent_kwargs, **kwargs_from_module)
                next_obs, reward, done, info = env.step(action)
                if callback:
                    kwargs_from_module = callback((obs, action, next_obs, reward, done))
            obs = next_obs
            total_reward += reward
            step += 1
...
def step_env_and_add_to_buffer(
    ...
    Args:
        ...
        agent_uses_low_dim_obs (bool): only valid if env is of type
            ...
            obs = env.get_last_low_dim_obs(). This is useful for rolling out an agent
            trained with low dimensional obs, but collect pixel obs in the replay buffer.
        grow_buffer (bool): If ``True``, stores new experience in the replay buffer.
    ...
    if isinstance(agent_obs, dict):
        agent_kwargs['goal'] = agent_obs['desired_goal']
        agent_obs = agent_obs['observation']
    action = agent.act(agent_obs, **agent_kwargs)
    next_obs, reward, done, info = env.step(action)
    if grow_buffer:
        replay_buffer.add(obs, action, next_obs, reward, done)
    kwargs_from_module = callback((obs, action, next_obs, reward, done)) if callback else {}
    return next_obs, reward, done, info, kwargs_from_module
...
def evaluate_final(
    ...
    """Evaluates the agent in the environment for n trials.

    Args:
        env (gym.Env): evaluation environment.
        agent (mbrl.planning.Agent): agent to be evaluated.
        offline_epochs (int): number of offline epochs completed.
        callback (Callable, optional): callback after each step.
        reset_callback (Callable, optional): callback called during optimization.
        work_dir (String, optional): path where data can be saved.
        video_recorder (optional): object that collects and saves trial videos.
        prefix (String): prefix for names of saved files.

    Returns:
        Dict: tuple containing evaluation metrics, averaged across trials.
    """
    
    all_rews, all_infos = [], []
    trajs = []
    for i, (state, goal) in enumerate(env.get_eval_cases()):
        obs = env.reset()
        obs = env.set_pos(state)
        obs = env.set_goal(goal)
        agent.reset()
        video_recorder.init()
        kwargs_from_module = reset_callback(obs) if reset_callback else {}
        done, tot_rew = False, 0.0
        traj = []
        while not done:
            agent_obs = obs['observation'] if isinstance(obs, dict) else obs
            agent_kwargs = dict(goal=obs['desired_goal']) if isinstance(obs, dict) else {}
            agent_kwargs.update(kwargs_from_module)
            action = agent.act(agent_obs, **agent_kwargs)
            next_obs, reward, done, info = env.step(action)
            kwargs_from_module = callback((obs, action, next_obs, reward, done)) if callback else {}
            traj.append(obs)
        ...
        video_recorder.save(f'{prefix}_offline_{i}')
        trajs.append(traj)
    np.save(work_dir+f'/{prefix}_trajs.npy', trajs)
    return {'success_rate': np.mean([r>0. for r in all_rews]), 'avg_reward': np.mean(all_rews),
            **{'avg_'+k:np.mean([a[k] for a in all_infos]) for k in all_infos[0]}}
