...
class Normalizer:
    ...
    def unnormalize(self, val: Union[float, mbrl.types.TensorType]) -> torch.Tensor:
        if isinstance(val, np.ndarray):
            val = torch.from_numpy(val).to(self.device)
    ...
    def load(self, results_dir: Union[str, pathlib.Path], key: str):
        """Loads saved statistics from the given path."""
        path = pathlib.Path(results_dir) / (key+'_'+self._STATS_FNAME)
        if not path.exists():
            return
    ...
    def save(self, save_dir: Union[str, pathlib.Path], key: str):
        """Saves stored statistics to the given path."""
        save_dir = pathlib.Path(save_dir)
        with open(save_dir / (key+'_'+self._STATS_FNAME), "wb") as f:
            pickle.dump(self.get_state(), f)

    def set_state(self, state):
        self.mean = torch.from_numpy(state["mean"]).to(self.device)
        self.std = torch.from_numpy(state["std"]).to(self.device)
...
def propagate_random_model(
    ...
    """
    # TODO: this might require seeeding, but seems to be unutilized
...
def powerlaw_psd_gaussian(
    ...
    if generator:
        std = torch.tile(s_scale.flatten(), (*size[:-1], 1))
        sr = torch.normal(torch.zeros_like(std), std, generator=generator)
        si = torch.normal(torch.zeros_like(std), std, generator=generator)
    else:
