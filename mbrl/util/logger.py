...
class MetersGroup(object):
    def __init__(self, file_name: Union[str, pathlib.Path], formatting: LogFormatType, writer):
    ...
    def _prepare_file(self, prefix: Union[str, pathlib.Path], suffix: str) -> pathlib.Path:
        file_path = pathlib.Path(prefix).with_suffix(suffix)
        self._csv_resume = True if file_path.exists() else False
        return file_path
    ...
    def _dump_to_tensorboard(self, data, step: int):
        # tensorboardX complains too much about NaNs ;)
        {self._writer.add_scalar(k, np.nan_to_num(v), step) for k, v in data.items()}
    ...
    def dump(self, step: int, prefix: str, save: bool = True, color: str = "yellow"):
        ...
        if save:
            ...
            self._dump_to_tensorboard(data, step)
        self._meters.clear()
...
class Logger(object):
    ...
    def register_group(
        ...
        if group_name in self._groups:
            ...
            return
        new_group = MetersGroup(self._log_dir / group_name, formatting=log_format, writer=self.writer)
        self._groups[group_name] = (new_group, dump_frequency, color)
        if group_name not in self._group_steps:
    ...
    def get_state(self):
        return self._group_steps

    def set_state(self, state):
        self._group_steps = state
